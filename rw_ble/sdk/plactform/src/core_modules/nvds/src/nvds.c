...
#define NVDS_PARAMETER_MAX_LENGTH   NVDS_LEN_BASIC_THRESHOLD  //70
...
#define NVDS_MAX_STORAGE_SIZE    0x200 // 512B
#endif //(NVDS_READ_WRITE == 1)

// NVDS Mapping

/// Magic number offset
#define NVDS_MAGIC_NUMBER_ADDRESS         0x2000
...
static void nvds_read(uint32_t address, uint32_t length, uint8_t *buf)
{
    ...
    ASSERT_ERR(((length) <= nvds_env.total_size));

    // Read the memory
    nvds_env.flash_read(nvds_env.flash_id, address, length, buf, NULL);
}
...
static void nvds_write(uint32_t address, uint32_t length, uint8_t *buf)
{
    ...
    // Read the memory
    nvds_env.flash_write(nvds_env.flash_id, address, length, buf, NULL);
}

static void nvds_erase(uint32_t address, uint32_t length)
{
    // Erase NVDS
    nvds_env.flash_erase(nvds_env.flash_id, address, length, NULL);
}

static void nvds_init_memory(void)
{
    // clear the device
    nvds_env.erase((uint32_t)NVDS_MAGIC_NUMBER_ADDRESS,0x1000 );//nvds_env.total_size
 ...
 ****************************************************************************************
 */
#define FLASH_TYPE_EMBEN_BK3435   0x3435   //yangyang merge for 3268 from 3435
uint8_t nvds_init(struct nvds_env_tag env)
{
  	//UART_PRINTF("nvds_init\r\n");
    uint8_t status = NVDS_OK;

    // Identify Flash memory
    nvds_env.flash_id = FLASH_TYPE_EMBEN_BK3435;
    if(nvds_env.flash_id == FLASH_TYPE_EMBEN_BK3435)
    {
        // Initialize the pointer to the NVDS
        nvds_env.nvds_space = env.nvds_space;

        // initialize the access functions
        nvds_env.read = &nvds_read;
			
	    nvds_env.flash_read = env.flash_read;
				
		nvds_env.flash_write = env.flash_write;
			
		nvds_env.flash_erase = env.flash_erase;
