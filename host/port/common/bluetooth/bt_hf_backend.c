...
result_t hf_cmd_cind_read(void);
...
static hfp_hf_app_t *g_hf_current_app_ptr =NULL;
...
static void hfp_hf_connected(bt_hfp_hf_session_h session, bt_app_ctx_h app_ctx)
{
    ...
    {   
        ...
        {
            os_printf("&HFP.M-Phone, judge role fail!\n\n");
            return;
        }        
    }
    else if(TWS_PRIM_SEC_SECOND == get_tws_prim_sec())  //TWS-S conn to TWS-M.
    {
        ...
        {
            os_printf("&HFP.S-Phone, judge role fail!\n\n");
            return;
        }        
    }
#endif
                                ...
                                bt_app_ctx_h app_ctx)
{
    ...
    {        
        ...
        if (app_ptr && (app_ptr != &g_hfp_hf_app))
        {
...
result_t hf_cmd_cind_read(void)
{
    os_printf("hf_cmd_cind_read,:%p\n",  g_hf_current_app_ptr);
    if (!g_hf_current_app_ptr || !g_hf_current_app_ptr->session)
        return UWE_NODEV;

    hf_set_flag(g_hf_current_app_ptr, APP_HFP_PRIVATE_FLAG_AT_CMD_PENDING);
    return bt_hfp_hf_cind_read(g_hf_current_app_ptr->session);
}
#endif
...
void hf_audio_handler(int mode)
{
    if (!g_hf_current_app_ptr)
        return;
...
#endif
#ifdef CONFIG_TWS
static uint8_t s_at_result_is_valid = FALSE;
#endif
static void hf_at_result_print(hfp_hf_app_t *app_ptr, const at_result_t *at_result)
{
    app_env_handle_t env_h = app_env_get_handle();
    ...
    #if HF_PRINTF_ENABLE
    os_printf("at_result->code:%d\r\n",at_result->code);
    #endif
#ifdef CONFIG_TWS
    s_at_result_is_valid = TRUE;
#endif
    ...
    case AT_RESULT_CIEV:
        {
            ...
            if(ciev->cind_code == CIND_CALL)
            {
                ...
                else
                {
                    os_printf("ciev0\r\n");
				#if TWS_HFP_ENABLE
                    jtask_stop(sys_hdl->tws_linein_task);
                    unset_tws_hfp_flag(TWS_HFP_FLAG_HFACK_HAPPEN);
				#endif
                    aud_PAmute_oper(1);
		      		app_wave_file_voice_num_set(NULL);
    ...
    if(s_at_result_is_valid)
    {
        ...
        )
        {
            ...
            {/* Let the TWS-SLAVE stoping the music immediately. */
                #if HF_PRINTF_ENABLE
                os_printf(" call happen \n");
                #endif
                
                tws_set_call_exist(TRUE);

                /* Let the TWS-MASTER stoping the music immediately. */
        ...
        else if((get_tws_prim_sec() == TWS_PRIM_SEC_PRIMARY)&& (tws_get_call_exist() == TRUE))
        {
            os_printf("call end:0x%x,0x%x\r\n", sys_hdl->flag_sm1,sys_hdl->flag_sm2);
