...
#ifdef CONFIG_TWS
extern void app_stereo_master_start_play(void);
#endif
...
#endif
#ifdef CONFIG_TWS
inline uint32 a2dp_single_check_flag(uint32 flag)
{
    return g_a2dp.flag & flag;
}
#endif
...
#ifdef CONFIG_TWS
static uint32 s_is_tws_m_conn_phone = 0; /* 0: TWS-M doesn't connect to the phone, 1: TWS-M has connected to the phone. */
uint32 is_tws_m_conn_phone(void)
{
    return s_is_tws_m_conn_phone;
}

void set_tws_m_conn_phone(uint32 f)
{
    s_is_tws_m_conn_phone = f;
}
    ...
    bt_app_ctx_h app_ctx)
{
    ...
    {        
        set_tws_m_conn_phone(1);
        
        os_printf("a2dp_connected:TWS_M connected to the Cell Phone.\r\n");
    }
    else    // TWS_Slaver
    {        
        os_printf("a2dp_connected: TWS_Slave connected to the TWS_Master.\r\n");
...
#ifdef CONFIG_TWS
    if (TWS_PRIM_SEC_PRIMARY == get_tws_prim_sec()) //TWS-M conn to phone.
    {   
        if (FALSE == tws_judge_piconet_role(&(sys_hdl->remote_btaddr)))
        {
            os_printf("&.M-Phone, judge role fail!\n\n");
            return;
        }        
    }
    else if(TWS_PRIM_SEC_SECOND == get_tws_prim_sec())  //TWS-S conn to TWS-M.
    {
        //tws_judge_piconet_role(&(sys_hdl->stereo_btaddr));
        if (FALSE == tws_judge_piconet_role(&(sys_hdl->stereo_btaddr)))
        {
            os_printf("000M-Phone, judge role fail!\n\n");
            return;
        }        
    }
#endif

    os_printf("a2dp_connected()\r\n");
#ifdef BT_ONE_TO_MULTIPLE
...
{   
    ...
    if(get_tws_prim_sec() == TWS_PRIM_SEC_PRIMARY)  // TWS_Master
    {
        set_tws_m_conn_phone(0);
        os_printf("TWS_M disconnected to the Cell Phone.\r\n");
    }
    else    // TWS_Slaver
    {
        os_printf("TWS_S disconnected to the TWS_M.\r\n");
    ...
    else
    {
        ...
        if ((NULL != app_ptr) && (app_ptr != &g_a2dp))
        {
...
#endif
#ifdef CONFIG_TWS
static uint32_t s_a2dp_rate = 44100;//default use 44.1k rate. 
void set_a2dp_rate(uint32_t a2dp_rate) 
{
    s_a2dp_rate = a2dp_rate;
}

uint32_t get_a2dp_rate(void) 
{
    return s_a2dp_rate;
}
#endif
...
#ifdef CONFIG_TWS
    set_a2dp_rate(rate);//for sync linein-adc/sbc-encode/a2dp rate.
#endif
    app_ptr->freq    = rate;
...
void a2dp_audio_handler(int32 mode)
{
    os_printf("a2dp_audio_handler(%d)\r\n", mode);
    if(1 == mode)
    {
    	aud_close();
...
void stream_start_handler(void *arg)
{
	...
	*/
    ...
    sbc_stream_start_init(app_ptr->freq);
    ...
    {
    #ifdef CONFIG_TWS // for sometimes,incoming call,when ===CALLSETUP=1, and then, nothing happened. Need to clear these flags. 
    ...
    {
    #ifdef CONFIG_TWS
        if(get_tws_prim_sec() == TWS_PRIM_SEC_PRIMARY)  /* Only TWS-M shall launch the avrcp connection with phone. */
        {            
    ...
    bt_app_ctx_h app_ctx)
{
    ...
    if( !bt_flag1_is_set( APP_FLAG_MUSIC_PLAY ))
    {
        ...
        if( bt_flag2_is_set( APP_FLAG2_STEREO_WORK_MODE ) )
        {
            ...
            {
                bt_flag2_operate( APP_FLAG2_STEREO_STREAMING, 1 );
...
#ifdef CONFIG_TWS
uint32_t get_a2dp_stream_start_flag(void)
{
	return(bt_flag1_is_set(APP_FLAG_MUSIC_PLAY|APP_BUTTON_FLAG_PLAY_PAUSE));
}
#endif
static void a2dp_stream_start_cb(bt_a2dp_sink_session_h session_h,
    bt_app_ctx_h app_ctx)
{
    ...
    if(bt_flag2_is_set(APP_FLAG2_STEREO_WORK_MODE))
    {
        os_printf("STEREO_WORK_MODE \n");
    }
    else
    {
        os_printf("NO STEREO_WORK_MODE \n");
    }
...
//	select_other_avrcp_app_pause_by_raddr(&(app_ptr->raddr));
	...
	{
	#ifdef CONFIG_TWS
		/*Because of idx = 0,which is phone,and idx = 1,it must be TWS_PRIM_SEC_SECOND */
		app_bt_write_sniff_link_policy(bt_sniff_get_handle_from_idx(0), 0);
		app_bt_write_sniff_link_policy(bt_sniff_get_handle_from_idx(1), 0);
	#else
		uint16_t handle;
		handle = bt_sniff_get_handle_from_raddr(&(app_ptr->raddr));
		app_bt_write_sniff_link_policy(handle, 0);
...
// ......???????lianxue.liu 01/06/2017
    ...
    if(!bt_flag1_is_set( APP_FLAG_HFP_CALLSETUP|APP_FLAG_HFP_OUTGOING)&& !a2dp_has_music())
    {
