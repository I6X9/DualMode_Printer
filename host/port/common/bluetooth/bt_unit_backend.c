    ...
    const btaddr_t *raddr, btclass_t uclass)
{
	...
	if ((BT_DEVCLASS_AUDIO_VIDEO == uclass[1]) && (0x26 == uclass[2]))
	{
			...
			)
		{
			...
			{
				//os_printf("reject_conn\n");
                os_printf(">[Accept a new acl link while auto connecting,and stop the auto connect task \r\n");        
				...
				//unit_acl_reject_stereo(app->unit, raddr,HCI_ERR_CONNECTION_REJECTED_DUE_TO_SECURITY_REASONS);
			}
			else
			{
				//os_printf("reject_stereo\n");
				unit_acl_reject_stereo(app->unit, raddr,HCI_ERR_CONNECTION_REJECTED_DUE_TO_LIMITED_RESOURCES);
			    return;
			}
		}
	    ...
	    {/* When the tws primary accepted the tws second connection request. */
	    	rc = bt_unit_acl_accept(app->unit, raddr);
	    }
       #endif
	}
	...
	#ifdef CONFIG_TWS
            {
            if(bt_flag1_is_set(APP_FLAG_A2DP_CONNECTION|APP_FLAG_LINEIN))
            {
                os_printf("rej_B, 0x%x\n", bt_flag1_is_set(APP_FLAG_A2DP_CONNECTION|APP_FLAG_LINEIN));
                unit_acl_reject_stereo(app->unit, raddr,HCI_ERR_CONNECTION_REJECTED_DUE_TO_LIMITED_RESOURCES);
                return;
            }
                app_bt_auconn_stop();
    ...
    const btaddr_t *raddr, uint32_t value)
{
    ...
    len = j_snprintf(buf, sizeof(buf), "bt_user_confirmation_request " BTADDR_FORMAT
...
#endif
#ifdef CONFIG_TWS
static void bt_beken_defined_evt_complete(bt_app_ctx_h app_ctx_h,
    hci_error_t status, const btaddr_t *raddr)
{
#if TWS_HFP_ENABLE
    os_printf(">[evt bk:%02x," BTADDR_FORMAT "\r\n",status,BTADDR(raddr));
    if(!get_tws_flag(TWS_FLAG_W4_SLAVE_CONNECT))
    {
        os_printf(">[tws the 2nd role connect prim role while sco\r\n");
        set_tws_flag(TWS_FLAG_W4_SLAVE_CONNECT);
...
static void bt_scan_enable_complete(bt_app_ctx_h app_ctx_h,
    hci_error_t hci_err)
{
    os_printf("bt_scan_enable_complete:0x%x, 0x%x\r\n",  hci_err, app_bt_get_disconn_event());
                                ...
                                result_t err)
{
...
#ifdef BEKEN_DEBUG
    os_printf("-----------------------------------\r\n");
    os_printf("| Bluetooth name: %s\r\n",app->unit->local_name);
    os_printf("| Bluetooth addr: " BTADDR_FORMAT "\r\n",BTADDR(&app->unit->hci_btaddr));
#ifdef BEKEN_OTA
    os_printf("| MCU version: 0x%x , DSP version: 0x%x \r\n", app_ota_get_version(IMAGE_MCU), app_ota_get_version(IMAGE_DSP));
#endif
    os_printf("-----------------------------------\r\n");
#endif
    ...
    if(app_env_check_HID_profile_enable())
    {
        err = bt_unit_set_class(app->unit, COMBO_SERVICE,COMBO_DEVSUBCLASS,dev_class); 
    }
    else
#endif
        //err = bt_unit_set_class(app->unit, SAMPLE_SERVICE, major_device_class,dev_class);

	err = bt_unit_set_class(app->unit, SAMPLE_SERVICE, BT_DEVCLASS_IMAGING,BT_DEVSUBCLASS_PRINTER);
    ...
    bt_unit_set_page_scan_type(app->unit,INTERLACED_SCAN);//STANDARD_SCAN
    bt_unit_set_inquiry_scan_type(app->unit,STANDARD_SCAN);//INTERLACED_SCAN

	//if(app_check_bt_mode(BT_MODE_1V1)) // 0x800:1.28s  page scan
...
static void bt_mode_change_notification(bt_app_ctx_h app_ctx_h,hci_error_t bt_mode)
{
    os_printf("BT.mode=%d,%d\n",s_last_mode,bt_mode);
    g_sniff_flag = bt_mode;
    if(0 == bt_mode && (s_last_mode != bt_mode))
    {
        s_last_mode = 0;
        //set_sw_register(SYS_DSP_RESTARTING);
        //start_dsp();
    }
    else
    {
                            ...
                            bt_unit_app_cbs_t *app_cbs)
{
    ...
    app_cbs->mode_change_notification_cb = bt_mode_change_notification;
#ifdef CONFIG_TWS
    app_cbs->beken_defined_evt_cb = bt_beken_defined_evt_complete;
#endif
#ifdef CONFIG_BLUETOOTH_SSP
