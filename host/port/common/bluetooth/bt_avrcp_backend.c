...
{
#ifdef CONFIG_TWS
    return g_avrcp_current_app && (g_avrcp_current_app->flag & APP_FLAG_AVCRP_CONNECTION);
#else
    ...
    bt_app_ctx_h app_ctx)
{
    ...
    if(bt_flag2_is_set(APP_FLAG2_STEREO_WORK_MODE))
        bt_unit_twscontrol_scan(HCI_NO_SCAN_ENABLE);
    else
        bt_unit_twscontrol_scan(HCI_PAGE_SCAN_ENABLE);
#else
#ifdef CONFIG_BLUETOOTH_AVRCP_TG
	jtask_stop(sys_hdl->app_save_volume_task);
	jtask_schedule(sys_hdl->app_save_volume_task,2000,(jthread_func)avrcp_volume_sync,(void *)NULL);
#endif	
#endif
                                    ...
                                    bt_app_ctx_h app_ctx)
{
    ...
    if (app_ptr && (app_ptr != &g_avrcp_app))
    {		
        avrcp_ex_app_uninit(app_ptr);
        avrcp_app_unregister(app_ptr);
        jfree(app_ptr);
...
void avrcp_volume_sync(void)
{
    ...
    if(g_avrcp_current_app && avrcp_current_support_vol_syn() && (!(g_avrcp_current_app->flag & APP_FLAG2_VOL_SYNC_OK) || (vol_flag==0)))
    {
        ...
        os_printf("avrcp_volume_sync:0x%x\r\n",volume);
        volume += 1;
	    ...
	    {
	        uint8 set_absolute_vol[2] = {TWS_VENDOR_DEP_SET_ABSOLUTE_VOL, 0};
	        set_absolute_vol[1] = get_tg_volume_value();
	        avrcp_ct_bkdata_send((void *)set_absolute_vol, 2);
	    }
    #endif
    }
#endif
...
/* for TWS_Slave send arcp_cmd to TWS_Master. */
void send_cmd_test(void *arg)
{
    ...
    if (avrcp_current_is_processing())
    {
        return;
    }
...
extern uint8 app_bt_tws_linein_src_enable(void);
#endif
static void avrcp_ct_connected(bt_avrcp_ctrl_session_h session,
    bt_app_ctx_h app_ctx)
{
    os_printf("avrcp_ct_connected(%d)\r\n",get_tws_prim_sec());
#ifdef TWS_CONFIG_LINEIN_BT_A2DP_SOURCE
	app_bt_tws_linein_src_enable();
#endif	
#ifdef CONFIG_TWS
    if (get_tws_prim_sec() == TWS_PRIM_SEC_PRIMARY) 
    {
...
extern void app_current_send_volume(void);
/* TWS-SLAVE self avrcp cmd process func. */
...
//    uint16_t *seq;
    ...
    {     
        ...
        #ifdef TWS_CONFIG_LINEIN_BT_A2DP_SOURCE
            if(!app_bt_slave_linein_get()&&(!app_is_linein_mode())) // for tws :slave
        #endif
        ...
        case AVC_OP_TWS_POWER_OFF: /* Just power down TWS-SLAVE self. */
            {
            #if 1
                tws_power_down();                
            #else
        ...
        case AVC_OP_TWS_CALL: /* TWS-SLAVE stop music immediately. */
            {
            tws_set_call_exist(TRUE);
            tws_clear_a2dp_buffer(); 

            app_button_type_set(APP_BUTTON_TYPE_HFP);
            }
            break;

        case AVC_OP_TWS_CALL_END: 
            {
        ...
        case AVC_OP_TWS_RE_SYNC:
            //os_printf("&. AVC_OP_TWS_RE_SYNC\r\n");
            break;
...
/* For TWS-MASTER sending avrcp cmd to TWS-SLAVE. */
void send_ct_cmd_test(void *arg)
{
    uint32_t tmp = (uint32_t)arg;
    avc_op_t avc_op = (avc_op_t)tmp;

//	os_printf("%s, 0x%x-%d\n", __func__, tmp, g_app_ct.is_used);
    ...
    if((avc_op != AVC_OP_VOLUME_DOWN)
        ...
        && (avc_op != AVC_OP_TWS_POWER_OFF)
        && (avc_op != AVC_OP_TWS_CALL)
        && (avc_op != AVC_OP_TWS_CALL_END)
        && (avc_op != AVC_OP_MATCH)
    )
    {
        return;
    ...
    bt_avrcp_send_pass_through_cmd(g_app_ct.session, g_app_ct.last_tid, avc_op,AVC_PRESS);

    g_app_ct.last_tid = (g_app_ct.last_tid + 1) % AVRCP_MAX_TRANSACTIONS;

    bt_avrcp_send_pass_through_cmd(g_app_ct.session, g_app_ct.last_tid, avc_op,AVC_RELEASE);

}

void avrcp_ct_bkdata_send(void *payload, uint16_t payload_size)
{
	//os_printf("avrcp_ct_bkdata_send 0x%x\r\n",*((uint8*)payload));	
    if((!g_app_ct.session) || (!payload))
    {
//        os_printf("%s, error! NULL, %p, %p\r\n", __func__, g_app_ct.session, payload);
        return ;
    }
    avrcp_send_bk_data(g_app_ct.session, g_app_ct.last_tid, payload, payload_size);
}

void avrcp_master_send_slave(uint16 cmd,uint8 param)
{
	if((get_tws_prim_sec() == TWS_PRIM_SEC_PRIMARY)
		&& bt_flag2_is_set(APP_FLAG2_STEREO_WORK_MODE))
 	{
		cmd |= param<<8;
		avrcp_ct_bkdata_send(&cmd, 2);
 	}
}

void avrcp_slave_send_master(uint16 cmd,uint8 param)
{
        ...
        && bt_flag2_is_set(APP_FLAG2_STEREO_WORK_MODE))
    {
        ...
        avrcp_send_bk_data(g_avrcp_app.session, g_avrcp_app.last_tid, &cmd, 2);
    }
}

void avrcp_get_custom_req(void *arg ,uint8_t len)
{
    uint8 *param=(uint8*)arg;
    app_tws_t app_tws_h = app_get_tws_handler();
    //os_printf("avrcp_get_custom_req(%d)\r\n", param[0]);

	switch (param[0])
	{
		case TWS_VENDOR_DEP_LINEIN_CMD:
			switch (param[1])
			{	
			#ifdef TWS_CONFIG_LINEIN_BT_A2DP_SOURCE
				case TWS_VENDOR_DEP_LINEIN_UNMUTE:
					linePlay = 1;
					bt_flag2_operate(APP_FLAG2_VOL_MUTE|AP_FLAG2_LINEIN_MUTE, 0);
					app_audio_restore();
					app_set_led_event_action(LED_EVENT_LINEIN_PLAY);
					break;
					
				case TWS_VENDOR_DEP_LINEIN_MUTE:
					linePlay = 0;
					bt_flag2_operate(APP_FLAG2_VOL_MUTE|AP_FLAG2_LINEIN_MUTE, 1);
				...
				case TWS_VENDOR_DEP_LINEIN_ATTACH:
					linePlay = 1;
					aud_PAmute_oper(1);
					app_bt_slave_linein_set(1);
                                #ifdef CONFIG_BLUETOOTH_AVDTP_SCMS_T
                                    unset_tws_flag(TWS_FLAG_A2DP_CONTENT_PROTECT);
                                #endif
					break;
					
				case TWS_VENDOR_DEP_LINEIN_DETACH:
		...
		case TWS_VENDOR_DEP_SHAREME_CMD:
			switch (param[1])
			{
				case TWS_VENDOR_DEP_SHAREME_LEFT:
				case TWS_VENDOR_DEP_SHAREME_RIGTH:
				case TWS_VENDOR_DEP_SHAREME_STEREO:
					app_tws_h->shareme = param[1]; 
					break;	
				default:
					break;	
			}
