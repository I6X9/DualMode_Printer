...
void timer_pwm1_pt2_isr(void)
{
    ...
    if ((tick_counter % (100 * 3)) == 0)
    {
        os_printf("SPR_EPCR %x\n", get_spr(SPR_EPCR_BASE));            
    }
#endif
...
#endif
    if (!bt_flag1_is_set(APP_FLAG_CALL_ESTABLISHED|APP_FLAG_HFP_CALLSETUP|APP_FLAG_SCO_CONNECTION)
    #ifdef CONFIG_TWS
...
#ifdef CONFIG_TWS
    if (get_tws_flag(TWS_FLAG_CONNECTING_SLAVE))
    {
        sw_mute_cnt ++;
        if (sw_mute_cnt > 500)
        {
...
#if TWS_SYNC_BY_OS_TIMER
    if(get_tws_prim_sec() == TWS_PRIM_SEC_PRIMARY)
    {
        //if(tick_counter % 1 == 0) // 10 ms
        {
            set_tws_flag(TWS_FLAG_SYNC_BY_OS_TIMER);
        }
    }
    else if(get_tws_prim_sec() == TWS_PRIM_SEC_SECOND)
    {
        //if(tick_counter % 100 == 0) // 1 s
        //set_tws_flag(TWS_FLAG_SYNC_BY_OS_TIMER);
    }
#endif

#if TWS_A2DP_AVOID_POP_ENABLE
	tws_a2dp_avoid_pop_sound_cnt_chk();
#endif

#if TWS_SCO_AVOID_POP_ENABLE
	tws_sco_avoid_pop_sound_cnt_chk();
#endif
    
#if TWS_A2DP_AVOID_SBC_NODE_LEVEL_TOO_LOW_ENABLE
	tws_a2dp_base_sbc_freq_adj_chk();
#endif

#if TWS_SCO_AVOID_MSBC_NODE_LEVEL_TOO_LOW_ENABLE
	tws_sco_base_msbc_freq_adj_chk();
#endif
