...
extern void usb_interrupt_enable(void);
            ...
            uint16_t wIntrRxValue,uint8_t bIntrUsbValue)
{
    ...
    MUSB_DPRINTF1("%s\r\n", __FUNCTION__);
    MUSB_DPRINTF("MGC_NoneControllerIsr\r\n");
//	MUSB_DPRINTF("isr usb\r\n");
    if (pSystem)
    {
			 ...
			 MUSB_pfTimerExpired pfExpireCallback)
{
    ...
    MUSB_DPRINTF1("%s\r\n", __FUNCTION__);
//    printf("MGC_NoneArmTimer: pfExpireCallback = 0x%p\r\n", pfExpireCallback);
...
uint32_t MUSB_NoneRunBackground(void)
{
    ...
    MUSB_DPRINTF1("%s\r\n", __FUNCTION__);
//    MUSB_DPRINTF("MUSB_NoneRunBackground: MGC_bNoneSystemCount = %d\r\n", MGC_bNoneSystemCount);
    for (bController = 0; bController < MGC_bNoneSystemCount; bController++)
    {
    	...
    	if ((pSystem->bBsrRequest > pSystem->bBsrRan) || bBsrOver)
    	{
    		...
    		{
        		MUSB_DPRINTF("MUSB_NoneRunBackground: pSystem->bBsrRequest = 0x%x, pSystem->bBsrRan = 0x%x, bBsrOver = 0x%x\r\n",
...
static void* MGC_NoneInitController(const MUSB_NoneController* pControllerInfo)
{
    ...
    uint32_t dwStatus;
    uint8_t* pBaseIsr = NULL;
    uint8_t* pBaseBsr = NULL;
...
//		    pSystem = NULL;
	    ...
	    /* try UCD init */
		MUSB_DPRINTF("MGC_NoneInitController: --------1\r\n");
//        printf("pControllerInfo->wType 3 = %d\r\n", pControllerInfo->wType);
	    pSystem.pController = MUSB_NewController(&MGC_gNoneUtils,
	        pControllerInfo->wType, pBaseIsr, pBaseBsr);
//        printf("pSystem.pController->wQueueLength = %d, pSystem.pController->wQueueItemSize = %d\r\n",
...
//		pSystem = NULL;
		...
		MUSB_DPRINTF("MGC_NoneInitController: --------2\r\n");
...
uint8_t MUSB_InitSystem(unsigned long dwBsrPriority)
{
    ...
    MUSB_DPRINTF("---MUSB_InitSystem fun----\r\n");
