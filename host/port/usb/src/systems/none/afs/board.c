			       ...
			       uint8_t** ppBaseIsr, uint8_t** ppBaseBsr)
{
    ...
    MUSB_DPRINTF1("%s\r\n", __FUNCTION__);
    MUSB_DPRINTF("MUSB_BoardInitController\r\n");
...
/*	MUSB_DPRINTF("aTimerWrapper:wtimercount=%x\r\n",wTimerCount);
...
void MGC_AfsTimerExpired(void)
{
	...
	wIndex = 0;
	pUds = MGC_apAfsUds[0];
	//MUSB_DPRINTF("exp:0x%p\r\n", MGC_apAfsUds[0]);

//    printf("MGC_AfsTimerExpired: pUds = 0x%p\r\n", pUds);
	if (pUds == NULL)
    {
...
//    printf("MGC_AfsTimerExpired: pUds = 0x%p, pWrapper = 0x%p\r\n", pUds, pWrapper);
//	MUSB_DPRINTF("++++ time=%x,bTimerStart=%d +++\r\n",pWrapper->dwTime,pWrapper->bTimerStart);
	if (pWrapper != NULL)
	{
        if (pWrapper->bTimerStart == 1)
        {
            ...
            {
//                printf("MGC_AfsTimerExpired: time up, pWrapper->pfExpired = 0x%p\r\n", 
//                    pWrapper->pfExpired);
    //			MUSB_DPRINTF("-----timer expiered, function is called------\r\n");
