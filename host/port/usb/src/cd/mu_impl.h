 ...
 */
#define MGC_DIAG(_bLevel, _pC, _pMsg) if(MGC_bDiagLevel >= _bLevel) \
    _pC->pSystemServices->pfPrintDiag(_pC->pSystemServices->pPrivateData, \
    _pMsg)
...
#define MGC_DIAG1(_bLevel, _pC, _pText, _dwNum, _bBase, _bJust) { \
    MGC_DiagMsg[0] = 0; \
    _pC->pUtils->pfMessageString(MGC_DiagMsg, MGC_DIAG_MSG_SIZE, _pText); \
    _pC->pUtils->pfMessageNumber(MGC_DiagMsg, MGC_DIAG_MSG_SIZE, _dwNum, _bBase, _bJust); \
    MGC_DIAG(_bLevel, _pC, MGC_DiagMsg); \
};
...
#define MGC_DIAG2(_bLevel, _pC, _pText1, _dwNum1, _pText2, _dwNum2, _bBase, _bJust) { \
    MGC_DiagMsg[0] = 0; \
    _pC->pUtils->pfMessageString(MGC_DiagMsg, MGC_DIAG_MSG_SIZE, _pText1); \
    _pC->pUtils->pfMessageNumber(MGC_DiagMsg, MGC_DIAG_MSG_SIZE, _dwNum1, _bBase, _bJust); \
    _pC->pUtils->pfMessageString(MGC_DiagMsg, MGC_DIAG_MSG_SIZE, _pText2); \
    _pC->pUtils->pfMessageNumber(MGC_DiagMsg, MGC_DIAG_MSG_SIZE, _dwNum2, _bBase, _bJust); \
    MGC_DIAG(_bLevel, _pC, MGC_DiagMsg); \
};
...
typedef struct
{
    ...
    uint8_t bStringIndex;
    uint8_t bStringIndexNumber;
} MGC_EnumerationData;
