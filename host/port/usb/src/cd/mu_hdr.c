 ...
 */
static uint8_t MGC_HdrcTestFifo(MGC_Port* pPort, uint8_t bEnd,
				uint8_t bDatum, uint16_t wCount)
{
    ...
    MGC_DIAG1(1, pImplController, "Testing FIFO on endpoint ", bEnd, 10, 0);

    for (wIndex = 0; wIndex < MGC_MIN(wCount, 64); wIndex++)
    {
	aTest[wIndex] = bDatum;
    }
    ...
    MGC_HdrcLoadFifo(pPort, bEnd, wCount, aTest);
    MGC_Write8(pBase, MGC_O_HDRC_TESTMODE, MGC_M_TEST_FIFO_ACCESS);
    MUSB_MemSet(aTest, 0, 64);
    do
    {
    ...
    {
	MGC_DIAG2(1, pImplController, "Error: loaded FIFO with ", wCount, " bytes, RxCount=",
	    wReadCount, 10, 0);
	bResult = FALSE;
    }
    wReadCount = MGC_MIN(wReadCount, 64);
    MGC_HdrcUnloadFifo(pPort, bEnd, wReadCount, aTest);
    if (bEnd)
    {
	...
	{
	    MGC_DIAG2(1, pImplController, "Error: FIFO Tx data=", bDatum, " Rx data=",
		aTest[wIndex], 10, 0);
	    bResult = FALSE;
	}
    }
...
#if 0
    /* odd sizes like 33, 34, 35 don't work... */
    if (!MGC_HdrcTestFifo(pPort, 0, 0xaf, 33))
    {
	return FALSE;
    }
    if (!MGC_HdrcTestFifo(pPort, 0, 0x55, 64))
    {
	return FALSE;
    }
#endif
...
uint8_t MGC_HdrcServiceDefaultEnd(MGC_Port* pPort, MGC_BsrItem* pItem)
{
    ...
    {
        MUSB_DPRINTF("MGC_M_CSR0_P_SENTSTALL\r\n");
        bError = TRUE;
    ...
    {
        MUSB_DPRINTF("MGC_M_CSR0_P_SETUPEND\r\n");
        pPort->wSetupDataIndex = 0;
        pPort->wSetupDataSize = 0;
#ifdef MUSB_STATS
    ...
    if (bError)
    {
        MUSB_DPRINTF("mgc_HdrcService_error\r\n");
        if (pPort->bIsHost)
        {
...
uint8_t MGC_HdrcServiceReceiveReady(MGC_Port *pPort, uint16_t wEndIndex, MGC_BsrItem *pItem)
{
    ...
    do
    {
        if (pPort->bIsHost)
        {
            ...
            {
                MUSB_DPRINTF("MGC_M_RXCSR_H_RXSTALL\r\n");
                status = MUSB_STATUS_STALLED;
            ...
            {
                MUSB_DPRINTF("MGC_M_RXCSR_ERROR:0x%x\r\n", wVal);
...
static void MGC_HdrcResumeOff(void* pParam, uint16_t wTimer)
{
    ...
    MUSB_DPRINTF("MGC_HdrcResumeOff\r\n");
    bReg = MGC_Read8(pBase, MGC_O_HDRC_POWER);
...
void MGC_HdrcSetPortTestMode(MGC_Port *pPort, MUSB_HsetPortMode eMode)
{
    ...
    case MUSB_HSET_PORT_RESET:
        MUSB_DPRINTF("MUSB_HSET_PORT_RESET\r\n");
        bReg = MGC_Read8(pBase, MGC_O_HDRC_POWER);
        MGC_Write8(pBase, MGC_O_HDRC_POWER, bReg | MGC_M_POWER_RESET);
