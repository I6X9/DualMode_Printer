...
/*
 * Try to discover the controller type
 */
...
//    pWrapper = (MGC_ControllerWrapper*)MUSB_MemAlloc(
//	sizeof(MGC_ControllerWrapper));

    MUSB_DPRINTF1("%s\r\n", __FUNCTION__);
    MUSB_DPRINTF("MUSB_NewController\r\n");
//	if (pWrapper==NULL)
		{
//			MUSB_DPRINTF("MUSB_NewController nomemeory2\r\n");
		}
//    if (pWrapper)
//    {
...
//    }
 ...
 //   if (pWrapper)
    ...
    }	/* if controller wrapper was "allocated" */
			      ...
			      )
{
    ...
    MUSB_DPRINTF1("%s\r\n", __FUNCTION__);
    MUSB_DPRINTF("MUSB_StartController\r\n");
    if (pController)
    {
        pImpl = (MGC_Controller*)pController->pPrivateData;
    }
...
uint32_t MUSB_StopController(MUSB_Controller* pController)
{
	...
	if (pImpl->pPort->pDmaController)
	{
	    pImpl->pPort->pDmaController->pfDmaStopController(
		pImpl->pPort->pDmaController->pPrivateData);
	}
#endif
	return pImpl->pfProgramStopController(pImpl);
    }
    return MUSB_STATUS_UNSUPPORTED;
}
#endif
...
{
    uint32_t dwStatus = MUSB_STATUS_INVALID_ARGUMENT;
    MGC_Controller* pImpl = NULL;
	...
	if (pImpl->pPort->pDmaController)
	{
	    pImpl->pDmaControllerFactory->pfDestroyDmaController(
...
MUSB_Port* MUSB_GetPort(uint16_t index)
{
	...
	#if 0
			ptemp = &(pWrapper->PortImpl);
			pEnumData = &(ptemp->EnumerationData);
			if (pEnumData->pDevice == NULL)
				MUSB_DPRINTF("---mmmmmm--\r\n");
	#endif
		}
    }
					      ...
					      uint32_t dwHighWater)
{
    ...
    MUSB_DPRINTF1("%s\r\n", __FUNCTION__);
    MUSB_DPRINTF("MGC_AllocateDynamicFifo: pPort->bIsHost = 0x%x\r\n", pPort->bIsHost);
    if (pPort->bIsHost)
    {
