 ...
 */
int MGC_FdrcIsr(void* pParam,uint16_t wIntrTxValue,uint16_t wIntrRxValue,uint8_t bIntrUsbValue)
{
    int result;
//	MUSB_DPRINTF("mgc fdrcisr\r\n");
...
//	MUSB_DPRINTF("MGC_O_FDRC_POWER:%x\r\n",power);
...
//	MUSB_DPRINTF("power = %x,npower=%x,devctl=%x,ndevctl=%x,pPort->bWantSession=%x\r\n",
//		power,nPower,devctl,nDevCtl,pPort->bWantSession);
...
//	MUSB_DPRINTF("wcsrval = %x,wcount=%x\r\n",wCsrVal,wCount);
...
//		MUSB_DPRINTF("--csr0 value:%x,bResult:%x--\r\n",wCsrVal,bResult);
				     ...
				     MGC_BsrItem* pItem)
{
	...
	//		MUSB_DPRINTF("MGC_FdrcServiceTransmitAvail,status=%x\r\n",status);
	    if (status)//出错
	    {
			/* reset error bits */
		...
		else    // device
		{
		    ...
		    */
		    ...
		    if (pEnd && pEnd->pRxIrp)
		    {
				...
				{
//				    os_printf("%p %x %x %x %x\r\n", pEnd->pRxIrp, pEnd->bBsrResidual, pEnd->bIsrResidual, bVal, bVal2);
