...
/**************************** GLOBALS *****************************/
// USBHostEnumerateDeviceState: 0: reserved
//                              1: set address
//                              2: udisk connect ok
//                              3: udisk connect failed
//                              4: udevice connect ok
//                              5: udevice connect failed
static uint8_t USBHostEnumerateDeviceState = 0;
...
static void MGC_HostSetDeviceAddress(void* pParam, uint16_t wTimer)
{
    ...
    MUSB_DPRINTF1("%s\r\n", __FUNCTION__);
    MUSB_DPRINTF("MGC_HostSetDeviceAddress\r\n");

    MGC_DIAG1(2, pController, "Setting device address ",
              pEnumData->bAddress, 10, 0);

    /* program the address (some DRCs need it even in host mode) */
    pPort->bFuncAddr = 0;
    pPort->pfProgramBusState(pPort);

    MUSB_DPRINTF("create and start request\r\n");
	...
	//os_printf("control transfer:state:%x,length:%x\r\n",tmp,pEnumData->Irp.dwOutLength);
}
...
static void MGC_HostGetConfig(void* pParam, uint16_t wTimer)
{
    ...
    MGC_DIAG1(2, pPort->pController, "Getting config descriptor, length=",
        pEnumData->Irp.dwInLength, 16, 4);
...
//    MUSB_DPRINTF("hotst get config====wLength:%x,%x\r\n");
    MUSB_StartControlTransfer(pPort->pInterfacePort, &(pEnumData->Irp));
}
 ...
 * Set Configuration
 */
static void MGC_HostSetConfiguration(void* pParam, uint16_t wTimer)
{
    MGC_Controller* pController = (MGC_Controller*)pParam;
    MGC_Port* pPort = pController->pPort;
    MGC_EnumerationData* pEnumData = &(pPort->EnumerationData);
...
static void MGC_HostGetDeviceStatus(void* pParam, uint16_t wTimer)
{
 ...
 //   MUSB_DeviceRequest* pRequest = (MUSB_DeviceRequest*)pEnumData->aSetupTx;
    ...
    MUSB_MemCopy(pEnumData->aSetupTx, MGC_aGetDeviceStatus,
                 sizeof(MGC_aGetDeviceStatus));
                         ...
                         const uint8_t** ppEntry)
{
    ...
    while (wIndex < wLength)
    {
        ...
        case MUSB_TARGET_SUBCLASS:
//            MUSB_DPRINTF("%s: MUSB_TARGET_SUBCLASS\r\n", __FUNCTION__);
            bData = pOperand[++wIndex];
            wIndex++;
        ...
        case MUSB_TARGET_PROTOCOL:
//            MUSB_DPRINTF("%s: MUSB_TARGET_PROTOCOL\r\n", __FUNCTION__);
            bData = pOperand[++wIndex];
            wIndex++;
        ...
        case MUSB_TARGET_ACCEPT:
//            MUSB_DPRINTF("%s: MUSB_TARGET_ACCEPT\r\n", __FUNCTION__);
...
//    MUSB_DPRINTF("mgc_hostenumerator--:%x\r\n",pEnumData->bState);
    uint32_t Ret = 1;
    ...
    if (pEnumData->Irp.dwStatus)
    {
		...
		MGC_DIAG2(1, pController, "Enumeration of device ", pEnumData->bAddress,
			" failed in state ", pEnumData->bState, 10, 0);
		MGC_DIAG2(1, pController, "\tbmRequestType=", pEnumData->Irp.pOutBuffer[0],
			"/bRequest=", pEnumData->Irp.pOutBuffer[1], 16, 2);
		...
		MGC_DIAG2(1, pController, "\twValue=", wValue, "/wIndex=", wIndex, 16, 4);
		wLength = pEnumData->Irp.pOutBuffer[6] | (pEnumData->Irp.pOutBuffer[7] << 8);
			MGC_DIAG2(1, pController, "\twLength=", wLength, "/Status=", pEnumData->Irp.dwStatus, 16, 8);
		if (++pEnumData->bRetries > MGC_MAX_ENUM_RETRIES)
		{
			MUSB_DPRINTF("MGC_HostEnumerator: error retry\r\n");
			...
			{
				MGC_DIAG(1, pController, "Restarting controller");
				pPort->bWantSession = FALSE;
				pPort->pfProgramBusState(pPort);
				pServices->pfArmTimer(pServices->pPrivateData,
					0, 10, FALSE, MGC_HostRetryEnum);
			}
#endif
			return Ret;
		}
    ...
    switch (pEnumData->bState)
    {
		case MGC_EnumStateSetAddress:
            USBHostEnumerateDeviceState = 1;
			MUSB_DPRINTF("%s: MGC_EnumStateSetAddress\r\n", __FUNCTION__);
	//            memory_usage_show();
		pEnumData->pDevice->bBusAddress = pEnumData->bAddress;
		if (!pPort->bIsMultipoint)
		{
	//        MUSB_DPRINTF("set address---\r\n");
			/* program the address (some DRCs need it even in host mode) */
...
//		memory_usage_show();
		...
		case MGC_EnumStateGetFullDevice:
			MUSB_DPRINTF("%s: MGC_EnumStateGetFullDevice\r\n", __FUNCTION__);
		pDeviceDesc = (MUSB_DeviceDescriptor*)pEnumData->aSetupRx;
	...
	//    MUSB_DPRINTF("$$$$$$$$$$$$ apconfigdescriptors alloc $$$$$$$$$$$$\r\n");
	...
	//    MUSB_DPRINTF("malloc size2===%d\r\n",tmp);
		if (pEnumData->pDevice->apConfigDescriptors)
		{
	        ...
	        pServices->pfArmTimer(pServices->pPrivateData, 0, 2, FALSE, MGC_HostGetConfig);
//MUSB_DPRINTF("malloc size2=== end\r\n");
	        break;
	    }
	    else
	    {
	    ...
	    pEnumData->pDevice->apConfigDescriptors[pEnumData->bIndex] = 
	        (MUSB_ConfigurationDescriptor*)wLength;
...
//		    MUSB_DPRINTF("pEnumdata->dwData = %x,wlengh=%x\r\n",pEnumData->dwData,wLength);
	        ...
	        pEnumData->pDevice->apConfigDescriptors[0] = (MUSB_ConfigurationDescriptor*)pDescriptorBuffer;
		    ...
		    */
		    	...
		    	bConfig++)
		    {
			    ...
			    pDriver = MGC_HostFindDriver(pPort->pHostClient, 
			    	pEnumData->pDevice, pConfig, (const uint8_t **) &pEntry);
			    if (pDriver)
			    {
	        ...
	        if (pDriver)
	        {
		    MGC_DIAG1(2, pController, "Found driver for device revision=", 
		        pEnumData->pDevice->DeviceDescriptor.bcdDevice, 16, 4);
	        }
	        else
	        {
		    MGC_DIAG1(2, pController, "No driver for device revision=", 
		        pEnumData->pDevice->DeviceDescriptor.bcdDevice, 16, 4);
	        }
	        MGC_DIAG2(2, pController, "VID=", 
		    pEnumData->pDevice->DeviceDescriptor.idVendor, "/PID=", 
		    pEnumData->pDevice->DeviceDescriptor.idProduct, 16, 4);
...
/*运行到此处时，刚完成get configuration(full)整个过程,接下来是set configuration()*/	
                    ...
                    {
                        USBHostEnumerateDeviceState = 2;
//                        msg_put(MSG_USB_HOST_ATTACH_UDISK_OK);
                    }
                    else
                    {
                        USBHostEnumerateDeviceState = 3;
...
//					MUSB_ERR_PRINTF("MGC_HostEnumerator: did not accept device\r\n");
//					else
//					{
                ...
                MUSB_pfHubEnumerationComplete pfHubEnumerationComplete)
{
    ...
    memory_usage_show();
//    MUSB_DPRINTF("++++++ start emunate malloc size:%d ++++++++++\r\n",sizeof(MGC_Device));
    if (pDevice)
    {
        ...
        pDevice->Device.pPrivateData = pDevice;
            pEnumData->dwData = 0;
        pEnumData->bAddress = bAddress;//设置address
        ...
        pEnumData->bFatalRetries = 0;
        pServices->pfArmTimer(pServices->pPrivateData,
            0, 40, FALSE, MGC_HostSetDeviceAddress);
        return TRUE;
    }
    else
    {
...
void MUSB_RejectDevice(MUSB_BusHandle hBus, MUSB_Device* pDevice)
{
    ...
    {
	//     MUSB_DPRINTF("++++++free apConfigDescriptors +++++++\r\n");
    ...
    {
	//    MUSB_DPRINTF("++++++free pDescriptorBuffer +++++++\r\n");
    ...
    if (pImplDevice)
    {
	//        MUSB_DPRINTF("++++++free MGC_Device struct :size:%d +++++++\r\n",sizeof(MGC_Device));
