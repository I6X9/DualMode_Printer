                   ...
                   uint8_t bContinue, void* pCurrentIrp)
{
    ...
    switch (pEnd->bRxTrafficType & MUSB_ENDPOINT_XFERTYPE_MASK)
    {
        ...
        wUnloadCount = 0;
        bComplete = TRUE;
        MGC_DIAG1(1, pPort->pController, "Internal error on end ", 
            pEnd->bLocalEnd, 10, 0);
        break;
    }
    /* unload FIFO (if anything the lower layer didn't unload) */
    MUSB_DPRINTF("wUnloadCount=%d\r\n", wUnloadCount);
            ...
            uint8_t bStatus, uint8_t bIsTx, void* pCurrentIrp)
{
 ...
 //           printf("IrpComplete[%s:%x]\r\n", (bIsTx) ? ("T") : ("R"), pIrp->pBuffer[0]);
 ...
 */
uint8_t MUSB_GetPipeHalt(MUSB_Pipe hPipe)
{
