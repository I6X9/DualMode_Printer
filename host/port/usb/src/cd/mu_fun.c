    ...
    if (MUSB_STATUS_OK == dwStatus)
    {
        if (pPort->pFunctionClient->pfInterfaceSet)
        {
            bOk = pPort->pFunctionClient->pfInterfaceSet(
                ...
                bIfaceIndex, bAltSetting, (MUSB_Pipe*)pPort->apPipe);
...
* Register a client
*/
extern unsigned long ulMGC_aDescriptorDataLen;
extern unsigned long ulMGC_aHighSpeedDescriptorDataLen;
extern const uint8_t *pMGC_aDescriptorData;
extern const uint8_t *pMGC_aHighSpeedDescriptorData;
				    ...
				    MUSB_FunctionClient* pClient)
{
    ...
    MGC_Pipe* pPipe;
    uint32_t dwStatus = MUSB_STATUS_OK;
    const MUSB_DeviceDescriptor* pDeviceDesc = (MUSB_DeviceDescriptor*)pClient->pStandardDescriptors;
    const MUSB_QualifierDescriptor* pQualifierDesc = (MUSB_QualifierDescriptor*)pClient->pHighSpeedDescriptors;
    uint8_t bNumConfigurations = pDeviceDesc->bNumConfigurations;
//	static uint32 pConfigDescriptor;
//	static uint32 MGC_Pipe_pointer[3];
//	static MGC_Pipe pipe[3];
    ...
    MUSB_DPRINTF("MGC_FunctionRegisterClient: pPort->apConfigDescriptors = 0x%p\r\n",
    		pPort->apConfigDescriptors);
//	pPort->apConfigDescriptors = (const MUSB_ConfigurationDescriptor**)&pConfigDescriptor;
    ...
    {
//   	pPort->apPipe = (MGC_Pipe**)MGC_Pipe_pointer;
...
//    MUSB_DPRINTF("\r\n");
//    MUSB_DPRINTF("dwStatus=0x%x\r\n", dwStatus);
//    MUSB_DPRINTF("apConfigDesc[0]=0x%p\r\n", pPort->apConfigDescriptors[0]);
...
//extern const uint8_t gHidReportDescriptor[];
//extern unsigned long ulgHidReportDescriptorLen;

extern const uint8_t gHidKeyboardReportDescriptor[];
extern unsigned long ulgHidKeyboardReportDescriptorLen;

extern const uint8_t gHidMouseReportDescriptor[];
extern unsigned long ulgHidMouseReportDescriptorLen;

extern const uint8_t gHidRMCVoiceReportDescriptor[];
extern unsigned long ulgHidRMCVoiceReportDescriptorLen;
...
uint8_t MGC_FunctionParseSetup(MGC_Port* pPort, uint8_t* pbStatus)
{
    ...
    *pbStatus = MUSB_STATUS_OK;
    ...
    MUSB_DPRINTF("MGC_FunctionParseSetup: pRequest = 0x%p\r\n", pRequest);
    if (pRequest && (MUSB_TYPE_STANDARD == (pRequest->bmRequestType & MUSB_TYPE_MASK)))
    {
        ...
        MUSB_DPRINTF("MGC_FunctionParseSetup: pRequest->bmRequestType = 0x%x, bRecip = 0x%x, wValue = 0x%x, wIndex = 0x%x\r\n",
                pRequest->bmRequestType, bRecip, wValue, wIndex);
    ...
    //        GPIO_Output(26, FALSE);
        ...
        case MUSB_REQ_GET_DESCRIPTOR:
                ...
                if (pScan < pScanLimit)
                {
                    ...
                    if (bSwitchDescriptor)
                    {
                        ...
                        case MUSB_DT_DEVICE:
                            ...
                            {
                                int i;
                                MUSB_DPRINTF("pPort->wSetupDataSize=0x%x\r\n", pPort->wSetupDataSize);
                                MUSB_DPRINTF("pPort->pSetupData:\r\n");
                                for (i=0; i<pPort->wSetupDataSize; i++)
                                    MUSB_DPRINTF("%02x ", pPort->pSetupData[i]);
                                MUSB_DPRINTF("\r\n");
                            }
                    ...
                    else
                    {
                        ...
                        if (0)
                        {
							int i;
                            MUSB_DPRINTF("no dance: pPort->wSetupDataSize=0x%x\r\n", pPort->wSetupDataSize);
                            MUSB_DPRINTF("no dance: pPort->pSetupData:\r\n");
                            for (i=0; i<pPort->wSetupDataSize; i++)
                                MUSB_DPRINTF("%02x ", pPort->pSetupData[i]);
                            MUSB_DPRINTF("\r\n");
                        }
            ...
            else if (MUSB_RECIP_INTERFACE == bRecip)
            {
                ...
                wWhich = wValue & 0x00ff;
                wIter = 0;
//                MUSB_DPRINTF("bType=%d\r\n", bType);
                switch (bType)
                {
                    case MUSB_DT_HID_REPORT:
//                        MUSB_MemCopy(pPort->pSetupData, gHidReportDescriptor, ulgHidReportDescriptorLen);
//                        pPort->wSetupDataSize = ulgHidReportDescriptorLen;
                        MUSB_DPRINTF("wIndex=%d\r\n", wIndex);
                        switch (wIndex)
                        {
            /*            	case 0:
...
//            MUSB_DPRINTF("SET_CONFIG:bUsbState=%d, wValue=%d, bIsHS=%d\r\n", pPort->bUsbState, wValue, pPort->bIsHighSpeed);
            bOurs = TRUE;
            pPort->wSetupDataSize = 0;
            bOK = FALSE;
...
//            MGC_FunctionChangeState(pPort, MUSB_SUSPENDED);
            pPort->wSetupDataSize = 0;
            break;
...
/*        bOurs = TRUE;
        wValue = ((((pRequest->wValue) & 0x00FF) << 8) | (((pRequest->wValue) & 0xFF00) >> 8));
        wIndex = ((((pRequest->wIndex) & 0x00FF) << 8) | (((pRequest->wIndex) & 0xFF00) >> 8));
        os_printf("VENDOR: pRequest->bRequest = 0x%x, wIndex = 0x%x, wValue = 0x%x\r\n",
                pRequest->bRequest, wIndex, wValue);
        switch (pRequest->bRequest)
        {
        case MUSB_VENDOR_CLEAR_FEATURE:
            pPort->wSetupDataSize = 0;
            break;
        
        default:
            MUSB_ERR_PRINTF("VENDOR: bRequest=0x%x\r\n", pRequest->bRequest);
            bOurs = FALSE;
            break;
        }*/
    }	/* END: if vendor request */
...
void MGC_FunctionChangeState(MGC_Port* pPort, MUSB_State State)
{
    ...
    if (pPort->bUsbState != State)
    {
		...
		if (pPort->pFunctionClient)//¾ÍÊÇ MGC_MsdFunctionClient
		{
