...
#if 0
static const uint8_t MGC_aMsdBotGetStringData[] =
{
...
#if 0
    /* loop through volumes and cancel relevant IRPs and reset to CBW stage */
    for (bIndex = 0; bIndex < pBot->bLunCount; bIndex++)
    {
	pVolume = &(pBot->aVolume[bIndex]);
	switch (pVolume->bTransferStage)
	{
	case MGC_MSD_BOT_TRANSFER_STAGE_CBW:
	    MUSB_CancelTransfer(&(pVolume->CbwIrp));
	    break;
	case MGC_MSD_BOT_TRANSFER_STAGE_DATAIN:
	case MGC_MSD_BOT_TRANSFER_STAGE_DATAOUT:
	    MUSB_CancelTransfer(&(pVolume->DataIrp));
	    break;
	case MGC_MSD_BOT_TRANSFER_STAGE_CSW:
	    MUSB_CancelTransfer(&(pBot->CswIrp));
	    break;
	}
				    ...
				    uint8_t bIsReady)
{
    ...
    if (bIsReady)
    {
		...
		else if (!pVolume->bAnnounced)
		{
			...
			MUSB_DPRINTF("+++ bGotMedium = %x +++\r\n",pVolume->bGotMedium);
		    if (pVolume->bGotMedium)
		    {
		    	MUSB_DPRINTF("$$$$$$$ MUSB_HfiMediumInserted fun\r\n");
...
//	uint32_t RetVal = 1;
    ...
    MUSB_DPRINTF1("%s\r\n", __FUNCTION__);
//    MUSB_DPRINTF("MGC_BotControlIrpComplete start\r\n");
    if (!pBot || (pBot != pBot->pSelf))
		return 1;

//    MUSB_DPRINTF("MGC_BotControlIrpComplete: pBot->bInit = 0x%x\r\n", pBot->bInit);
    if (pBot->bInit)
    {
		/* we must be initializing, so it is GetMaxLUN */
		pBot->bInit = FALSE;
		/* device can stall it, meaning 1 LUN; otherwise a 1-byte "top LUN" */
//		pBot->bLunCount = (MUSB_STATUS_OK == pIrp->dwStatus) ?
//		    (pBot->aData[0] + 1) : 1;
//only support one LUN for memory limitation
		pBot->bLunCount = 1;
#if 0
		/* get lang IDs */
		pBot->bGetLang = TRUE;
		MUSB_MemCopy(pBot->aSetup, MGC_aMsdBotGetStringData,sizeof(MGC_aMsdBotGetStringData));
		pBot->ControlIrp.dwInLength = 32;
		dwStatus = MUSB_StartControlTransfer(pBot->pDevice->pPort, &(pBot->ControlIrp));
    }
    else if (pBot->bGetLang)
    {
		pBot->bGetLang = FALSE;
		/* get serial # from first lang ID */
		pBot->bGetSerial = TRUE;
		pBot->aSetup[2] = pBot->pDevice->DeviceDescriptor.iSerialNumber;
		MUSB_MemCopy(&(pBot->aSetup[4]), &(pBot->aData[2]), 2);
		dwStatus = MUSB_StartControlTransfer(pBot->pDevice->pPort, &(pBot->ControlIrp));
	}
    else if (pBot->bGetSerial)
    {
		pBot->bGetSerial = FALSE;
#endif
...
//即函数 : MGC_ScsiCmdSetCreateInstance()				
				...
				{
//                    MUSB_DPRINTF("MGC_BotControlIrpComplete: MGC_MsdBotVolumeInit\r\n");
				    pVolume->pBot = pBot;
				    pVolume->bLun = bIndex;
...
//发送完成后,会产生中断,在BSR中则会调用 pVolume->CbwIrp.pfIrpComplete = MGC_BotCbwComplete;
...
#if 0
		/* re-start command */
		MUSB_StartTransfer(&(pBot->pCurrentVolume->CbwIrp));
#endif
#if 1
	...
	{
	    MUSB_DIAG_STRING(1, "MSD/BOT: device failed reset; probably fatal");
	}
#endif
    }
	return 0;
...
static uint32_t MGC_BotCbwComplete(void* pParam, MUSB_Irp* pIrp)
{
    ...
    MUSB_DPRINTF1("%s\r\n", __FUNCTION__);
 //  MUSB_DPRINTF("--mgc botcbw complete----\r\n");
    if (!pBot || (pBot != pBot->pSelf))
		return RetVal;
...
static uint32_t MGC_BotDataComplete(void* pParam, MUSB_Irp* pIrp)
{
    ...
    MUSB_DPRINTF1("%s\r\n", __FUNCTION__);
//	MUSB_DPRINTF("--mgc bot data complete--\r\n");
...
static uint32_t MGC_BotCswComplete(void* pParam, MUSB_Irp* pIrp)
{
    ...
    MUSB_DPRINTF1("%s\r\n", __FUNCTION__);
	RetVal = 1;
//MUSB_DPRINTF("---mgc bot csw complete---\r\n");
	if (!pBot || (pBot != pBot->pSelf))
		return RetVal;
					   ...
					   uint8_t bAllowDma)
{
    ...
    if (bCmdLength)
    {
		...
		bOk = MGC_BotProtocolSendCmd(pBot, pVolume, pVolume->bLun, aCmdBuffer,
		    ...
		    FALSE, MGC_MsdBotDataTransferComplete, bAllowDma, 0);
