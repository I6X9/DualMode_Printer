...
MUSB_RamFileInfo* MUSB_RamFileCreate(uint32_t dwSize, const char* pszName)
{
    ...
    if (pBuffer)
    {
        MUSB_MemSet(pBuffer, 0, dwSize);
	pItem = (MUSB_RamFileInfo*)MUSB_MemAlloc(sizeof(MUSB_RamFileInfo));
	if (pItem)
	{
            MUSB_MemSet(pItem, 0, sizeof(MUSB_RamFileInfo));
	    pItem->dwSize = dwSize;
	...
	{
        MUSB_ERR_PRINTF("MUSB_RamFileCreate: MUSB_MemAlloc 2 failed\r\n");
    ...
    {
		MUSB_ERR_PRINTF("MUSB_RamFileCreate: MUSB_MemAlloc 1 failed\r\n");
    }
