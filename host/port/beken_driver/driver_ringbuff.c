...
uint16 DRAM_CODE rb_get_used_size(driver_ringbuff_t *rb)
{
...
/* calc the none-used size in the rb. */
int16 DRAM_CODE rb_get_buffer_size(driver_ringbuff_t *rb)
{
    return (rb->buffer_len - rb_get_used_size(rb));
}
...
#ifdef CONFIG_TWS
    if((debug != AEC_RX_FILL)&&(debug != AEC_TX_FILL)&&(debug != ADC_FILL))
#else
...
#if(CONFIG_AUD_FADE_IN_OUT == 1)
    {
        ...
        if(get_aud_fade_in_out_state() & (AUD_FADE_IN | AUD_FADE_OUT ))
        {
            for(j = 0; j < (size >> 2); j++)
            {
                ...
                *(smpl16 + 2*j) = smpl32_L >> AUD_FADE_SCALE_LEFT_SHIFT_BITS;
                *(smpl16 + 2*j + 1) = smpl32_R >> AUD_FADE_SCALE_LEFT_SHIFT_BITS;
            }
        }
    }
#endif
