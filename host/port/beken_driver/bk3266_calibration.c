...
void sar_adc_enable(uint32_t channel, uint32_t enable)
{
    if(enable)
    {
        ...
        reg_bit_set((uint32_t)&BK3000_PMU_GATE_CFG, 12, 1);
        #if 0//旧版本BK3268
        ...
        #else//新版本BK3268
        reg_bits_set((uint32_t)&BK3000_SARADC_ADC_CONF, 2, 5, channel); //SAR-ADC channel set        
        #endif
        ...
        while(i--)
        {
            ...
            while(t-- && (BK3000_SARADC_ADC_CONF & bit_SARADC_CONF_FIFO_EMPTY));
...
void tx_calibration_deinit(void)
{
    ...
    xvr_reg_0x0c_bak = (xvr_reg_0x0c_bak & 0xfffffc00) | (XVR_ANALOG_REG_BAK[0x0c] & 0x000003ff);
...
void rx_bpf_dc_offset_calibration(void)
{
	uint32_t reg0x07, reg0x25, adci, adcq;
	int32_t  x1, x2, y1, y2;

	reg0x07 = xvr_reg_get(ADDR_XVR_0x07);
	reg0x25 = xvr_reg_get(ADDR_XVR_0x25);

	xvr_reg_bits_set(ADDR_XVR_0x07, 11, 13, 7);
	xvr_reg_bit_set(ADDR_XVR_0x07, 17, 0);
	xvr_reg_bit_set(ADDR_XVR_0x25, 13, 1);

	xvr_reg_bit_set(ADDR_XVR_0x08, 5, 0);		//Set dacsgni=0
	xvr_reg_bit_set(ADDR_XVR_0x08, 4, 0);		//Set dacsgnq=0
	xvr_reg_bits_set(ADDR_XVR_0x08, 24, 27, 0);	//Set daci<3:0>=0
	xvr_reg_bits_set(ADDR_XVR_0x08, 0, 3, 0);  	//Set dacq<3:0>=0
	xvr_reg_bit_set(ADDR_XVR_0x08, 6, 0);		//Set pwddcadc=0

	delay_ms(1);

	adcq = reg_bits_get(ADDR_GPIO_0x3C, 3, 5);
	adci = reg_bits_get(ADDR_GPIO_0x3C, 0, 2);

	os_printf("%s: adcq = %d, adci = %d\n", __func__, adcq, adci);
	...
	xvr_reg_bit_set(ADDR_XVR_0x08, 5, (x1 < 0));
	xvr_reg_bit_set(ADDR_XVR_0x08, 4, (x2 < 0));
	...
	xvr_reg_bits_set(ADDR_XVR_0x08, 24, 27, x1);
	xvr_reg_bits_set(ADDR_XVR_0x08, 0, 3, x2);

	xvr_reg_set(ADDR_XVR_0x07, reg0x07);
	xvr_reg_set(ADDR_XVR_0x25, reg0x25);
}
