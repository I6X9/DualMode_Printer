...
{     
    ...
    dma_control_word = ((dma_config->request_select & 0x0f) <<16) + 
...
#if CONFIG_ADC_DMA
uint32 dma_get_dst_data(uint8 channel, uint8 *buf, uint32 len)
{
    uint32 dst_reg = BK3000_DMA_BASE_ADDR + (8*(channel+1) + 3)*4;
    uint32 dst_cur_reg = BK3000_DMA_BASE_ADDR + (0x40+2*channel+1)*4; 
    uint32 dst_top_reg = BK3000_DMA_BASE_ADDR + (8*(channel+1) + 6)*4;
    uint32 dst_bottom_reg = BK3000_DMA_BASE_ADDR + (8*(channel+1) + 7)*4;  
    uint32 dst_reg_val = (*((volatile unsigned long *) dst_reg));
    uint32 dst_cur_reg_val = (*((volatile unsigned long *) dst_cur_reg)); 
    uint32 dst_top_reg_val = (*((volatile unsigned long *) dst_top_reg));
    uint32 dst_bottom_reg_val = (*((volatile unsigned long *) dst_bottom_reg));
    uint32 cur_len = 0;
    uint32 copy_len = 0;
	
    if (dst_reg_val >= dst_cur_reg_val) {
        if (dst_reg_val + len > dst_bottom_reg_val) {
            uint32 dist_len = 0;
            copy_len = dst_bottom_reg_val - dst_reg_val + 4;
            memcpy(buf, (uint8*)dst_reg_val, copy_len);
            len -= copy_len;
            buf += copy_len;  
            
            dist_len = dst_cur_reg_val - dst_top_reg_val;
            len = (dist_len > len) ? len: dist_len;
            memcpy(buf, (uint8*)dst_top_reg_val, len);
            copy_len += len;
            (*((volatile unsigned long *) dst_reg)) = dst_top_reg_val + len;
        } else {
            copy_len = len;
            memcpy(buf, (uint8*)dst_reg_val, copy_len);
            (*((volatile unsigned long *) dst_reg)) = dst_reg_val + len;
        }
    } else if (dst_reg_val < dst_cur_reg_val) {
        cur_len = dst_cur_reg_val - dst_reg_val;
        copy_len = (cur_len > len)? len : cur_len;
        memcpy(buf, (uint8*)dst_reg_val, copy_len);
        (*((volatile unsigned long *) dst_reg)) = dst_reg_val + copy_len;
    }   
...
uint32 dma_get_dst_data_size(uint8 channel)
{
//os_printf("D.S%d\n", channel);
#if 0
    uint32 dst_reg = BK3000_DMA_BASE_ADDR + 0x4c;//(8*(channel+1) + 3)*4;
    uint32 dst_cur_reg = BK3000_DMA_BASE_ADDR + 0x10c;// (0x40+2*channel+1)*4; 
    uint32 dst_top_reg = BK3000_DMA_BASE_ADDR + 0x58;//(8*(channel+1) + 6)*4;
    uint32 dst_bottom_reg = BK3000_DMA_BASE_ADDR + 0x5c;//(8*(channel+1) + 7)*4;  
    uint32 dst_reg_val = (*((volatile unsigned long *) dst_reg));
    uint32 dst_cur_reg_val = (*((volatile unsigned long *) dst_cur_reg)); 
    uint32 dst_top_reg_val = (*((volatile unsigned long *) dst_top_reg));
    uint32 dst_bottom_reg_val = (*((volatile unsigned long *) dst_bottom_reg));
    ...
    if(dst_reg_val >= dst_cur_reg_val) 
    {
        if(dst_reg_val + len > dst_bottom_reg_val) 
        {
            ...
            {
                //os_printf("====1===1=====1===1\r\n");
                (*((volatile unsigned long *) dst_reg)) = dst_top_reg_val + copy_len;
                copy_len = 0;
            }
        } 
        else 
    ...
    {
        copy_len = dst_cur_reg_val - dst_reg_val;
        if(copy_len >= len)
        {
            //os_printf("====3===3=====3===3\r\n");
    ...
    if (dst_reg_val >= dst_cur_reg_val) 
    {
        ...
        if (0)//(copy_len >= 1024)
        {
            //os_printf("=====clear buffer11===:%x,%x,%x,%x\r\n",dst_cur_reg_val,dst_reg_val,dst_bottom_reg_val,dst_top_reg_val);
            if(dst_cur_reg_val == dst_top_reg_val)
                (*((volatile unsigned long *) dst_reg)) = dst_bottom_reg_val;
            else
                (*((volatile unsigned long *) dst_reg)) = dst_cur_reg_val - 4;
