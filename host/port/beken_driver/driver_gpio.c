...
void gpio_button_wakeup_enable(void)
{
    ...
    linein_cfg_t *linein_cfg = get_line_in_config();

    BK3000_GPIO_INTEN = 0;
    if(linein_cfg->enable)
    {
        BK3000_GPIO_INTSTA |= (1 << linein_cfg->pin);
        if(linein_cfg->high_flag)       
            BK3000_GPIO_INTLV  |= (1 << linein_cfg->pin);
        else
            BK3000_GPIO_INTLV  &= ~(1 << linein_cfg->pin);
        BK3000_GPIO_INTEN  |= (1 << linein_cfg->pin);      
    }
    ...
    BK3000_GPIO_INTLV &= ~(1<<22);             // low level detect of GPIO22 for bk3268 shutdown PIN;
