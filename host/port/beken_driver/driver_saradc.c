...
void saradc_init( int mode, int channel, int div )
{
   ...
   /* Try to reset SARADC ??? */
    ...
    else if(ADC_CHANNEL_10 == channel)
    {
        BK3000_GPIO_22_CONFIG = 0x08; //0x0C 
    }    
    
    reg =  mode
            |(channel << sft_SARADC_CHANNEL) //bit5-2
...
static uint16 bat_adc_filter(uint16 data)
{
    ...
    os_printf("+++adc:%d(mV),avr:%d(mV),aud_size:%d\r\n",value_buf[4],value_avr_adc,aud_get_buffer_used_size());
...
void saradc_chnl_scanning(uint32 step)
{
    ...
    if(!(BK3000_SARADC_ADC_CONF&SARADC_FIFO_EMPTY))
    {
        ...
        else if((0 == app_h->button_ad_enable)/*(app_h->button_ad_channel==ADC_CHANNEL_NULL)*/||( app_h->low_detect_channel == saradc_chn))  
        {
    ...
    else if(app_h->button_ad_enable) //(ADC_CHANNEL_NULL != app_h->button_ad_channel) //timer:10ms
    {//ADC KEY DET
