...
#ifdef CONFIG_IRDA
CONST static uint32 IR_Code_MSG[][4]=
{
    {MSG_KEY_PP,                                    0x45,                                                BUTTON_CLICK,                                                                                                                       BUTTON_TYPE_NON_HFP},    
    {MSG_KEY_HFP_ACK,                       0x45,                                                 BUTTON_CLICK,                                                                                                                      BUTTON_TYPE_HFP},  
    {MSG_KEY_NEXT,                              0x46,                                                BUTTON_LONG_PRESS,                                                                                                          BUTTON_TYPE_NON_HFP},
    {MSG_KEY_PREV,                              0x56,                                                BUTTON_LONG_PRESS,                                                                                                           BUTTON_TYPE_NON_HFP},
    {MSG_KEY_NUM_REDIAL,                0x45,                                                BUTTON_DOUBLE_CLICK,                                                                                               BUTTON_TYPE_ALL},
    {MSG_KEY_VOL_DOWN,                  0x14,                                                BUTTON_CLICK|BUTTON_REPEAT|BUTTON_DOUBLE_CLICK,                              BUTTON_TYPE_ALL},
    {MSG_KEY_MATCH,                         0x16,                                                BUTTON_CLICK,                                                                                                                   BUTTON_TYPE_ALL},
    {MSG_KEY_MODE,                        0x18,                                                BUTTON_CLICK,                                                                                                       BUTTON_TYPE_NON_HFP},
    {MSG_KEY_FORWARD,                   0x20,                                              BUTTON_CLICK,                                                                                                                   BUTTON_TYPE_NON_HFP},
};
#endif
#ifdef CONFIG_CRTL_POWER_IN_BT_SNIFF_MODE
...
void buttonInit(void)
{
    ...
    app_h->button_ad_enable = 0;
    for(i = 0; i < sizeof(buttonConfigEvent)/4; i++)
    {
        // AD key
        adc_enable = env_h->env_cfg.button_code[i] & BUTTON_IS_ADC_MASK;
        if(adc_enable == BUTTON_IS_ADC_MASK)
        {
            app_h->button_ad_enable = 1;
            app_h->button_ad_channel=(uint8) ((env_h->env_cfg.button_code[i] & BUTTON_AD_CHANNEL_MASK)>>BUTTON_MODE_ADC_CHAN_SHIFT);
	     app_h->button_adc_sum = (uint8)((env_h->env_cfg.button_code[i] & BUTTON_MODE_ADC_SUM_MASK) >> BUTTON_MODE_ADC_SUM_SHIFT);
            for(j = BUTTON_MODE_ADC_KEY_SHIFT; j < BUTTON_MODE_ADC_SUM_SHIFT; j ++)
            {
                if(env_h->env_cfg.button_code[i] & (1<<j))
                {
                    app_h->button_code[i] |= (1<<(j+18));//ADkey0-ADkey8:bit23-bit31.
                }
            }
        }
        else// IO key
...
//    BK3000_A5_CONFIG |= 0x02; 	 //配置ADC KEY使能
//    BK3000_A5_CONFIG |= 0x20; 	 //配置ADC KEY使能
}
...
uint32  buttonDetectAD(void)
{
    ...
    if(app_h->button_ad_enable) //( app_h->button_ad_channel !=ADC_CHANNEL_NULL)
    {
        if(adc_val < 40)	  //  s1 0R
        {
            keyAdMap = ADKEY0;
...
uint32 buttonDetect(void)
{
    ...
    if(keyValue && syspwr_cpu_halt)
        app_exit_sniff_mode();
#endif
    //if(keyValue&&(keyValue != (1<<DEFAULT_WAKEUP_PIN)))
...
void buttonMsgCompare(uint32 keyAction,uint32 keyValue)
{
    ...
    app_handle_t app_h = app_get_sys_handler();
#ifdef CONFIG_IRDA
     for(i=0;i<sizeof(IR_Code_MSG)/16;i++)
    {
        if((IR_Code_MSG[i][2]&keyAction)
            &&(IR_Code_MSG[i][1]==keyValue)
            &&(IR_Code_MSG[i][3]&app_h->button_type))
        {
            msg_put(IR_Code_MSG[i][0]);
            return;
        }
    }
#endif
