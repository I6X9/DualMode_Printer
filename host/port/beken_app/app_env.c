...
#ifdef CONFIG_TWS
extern int app_button_clear_memory( void );
#endif
...
inline void set_tws_env_stereo_role(uint8 sr)
{
    ...
    {
        app_env.env_data.stereo_role = sr;
    }
}

inline uint8 get_tws_env_stereo_role(void)
{
    return app_env.env_data.stereo_role;
}

inline uint8 clear_tws_env_stereo_role(void)
{
    app_env.env_data.stereo_role = TWS_PRIM_SEC_UNDEFINED;
}
#endif
...
#ifdef CONFIG_TWS
    os_printf("stereo role: %d\r\n", get_tws_env_stereo_role());

    for( j = 0; j < 6; j++ )
    {
        sprintf( &addr[j*2], "%02x", env_h->env_data.stereo_key.btaddr.b[5-j]);
    }
    for( j=0; j < 16; j++)
        sprintf( &linkkey[j*2], "%02x", env_h->env_data.stereo_key.linkkey[j]);
    addr[13] = 0;
    linkkey[33] = 0;

    os_printf("stereo key_pair used: %02x, addr: 0x%s, linkkey: 0x%s\r\n",
                 env_h->env_data.stereo_key.used, addr, linkkey);
...
void app_env_cali_data_init(void)
{
    ...
    s_env_voltage_cali_data.cali_dig_1p2v = 2;

    result = eFuse_read(cali_data,16,8);
    ...
    if(result)
    {
        ...
        //os_printf("===get calibration data:%x %x %x %x;%x %x %x %x \r\n",cali_data[0],cali_data[1],cali_data[2],cali_data[3],cali_data[4],cali_data[5],cali_data[6],cali_data[7]);
        s_env_charge_cali_data.charger_vlcf = cali_data[0]>>2;
        s_env_charge_cali_data.charger_icp = ((cali_data[0]&3)<<3)  + (cali_data[1]>>5);
        s_env_charge_cali_data.charger_vcv = cali_data[1]&0x1f;
         s_env_voltage_cali_data.cali_dig_1p2v = (cali_data[2]&0xc0)>>6;
        s_env_saradc_cali_data.sar_adc_dc = (cali_data[2]&0x1f) + 496;
        s_env_saradc_cali_data.sar_adc_4p2 = cali_data[3] + 768;
        s_env_audio_dc_offset_0.dac_l_dc_offset[0] = cali_data[4];
        s_env_audio_dc_offset_0.dac_r_dc_offset[0] = cali_data[5];
        s_env_audio_dc_offset_1.dac_l_dc_offset[0] = cali_data[6];
        s_env_audio_dc_offset_1.dac_r_dc_offset[0] = cali_data[7];
		#if 0
        os_printf("vlcf:%x,icp:%x,vcv:%x; dc:%x,4p2:%x; 0:[%x,%x],1:[%x,%x]\r\n",
            ...
            s_env_audio_dc_offset_0.dac_l_dc_offset[0],s_env_audio_dc_offset_0.dac_r_dc_offset[0],
            s_env_audio_dc_offset_1.dac_l_dc_offset[0],s_env_audio_dc_offset_1.dac_r_dc_offset[0]);
		#endif
    }
...
void app_env_init( void )
{
    ...
    {
        flash_write_data((uint8*)&env_h->env_cfg.bt_para.device_addr, FLASH_CHANGED_BT_ADDR, 6);
    }
    ...
    {
        flash_write_data((uint8*)&env_h->env_cfg.bt_para.device_name[0], FLASH_CHANGED_BT_ADDR + 6, 32);
    }
	struct printer_env user_env;

	flash_read_data(&user_env.bt_name[0],(uint32)RPINTER_ENV_ADDR, 32);
	
	int useri=0;
	int usern=0;
	
	 for(useri=0; useri<32; useri ++) // 32
	{
		 if((user_env.bt_name[useri] != 0xff)&&(user_env.bt_name[useri] != 0x00))
			 usern++;
	}
	if((user_env.bt_name[0]!=0x00)&&(user_env.bt_name[0]!=0xff)){
		
		memset((uint8*)&env_h->env_cfg.bt_para.device_name[0],0,32);
		memcpy((uint8*)&env_h->env_cfg.bt_para.device_name[0],&user_env.bt_name[0],usern);
	}
    ...
    if(env_h->env_cfg.used == 1)
    {
        if ((env_h->env_cfg.system_para.system_flag & APP_ENV_SYS_FLAG_UART01DBG)
            ||(env_h->env_cfg.system_para.system_flag & APP_ENV_SYS_FLAG_UART67DBG))
    	{
    	#if (CONFIG_DEBUG_PCM_TO_UART == 1)
    ...
    saradc_set_lowpower_para(env_h->env_cfg.system_para.lp_interval,
                                ...
                                env_h->env_cfg.system_para.lp_pd_threshold );
/*
    a7_val = BK3000_A7_CONFIG;
    a7_val &= ~0x60000000;
    a7_val |= ((( env_h->env_cfg.system_para.system_flag & APP_ENV_SYS_FLAG_MIC_BIAS ) >> 13) << 29);
    BK3000_A7_CONFIG = a7_val;
*/
    //FIXME: liaixing
...
#endif
    if(1) //设置shutdown唤醒GPIO通道
    {
        uint32_t wakeup_pin = env_h->env_cfg.system_para.wakup_pin;

        wakeup_pin = wakeup_pin < 0x1F ? wakeup_pin : 0x1F;
        wakeup_pin = BK3000_GPIO_MAPPING(0, wakeup_pin);

        BK3000_XVR_REG_0x04 = XVR_analog_reg_save[4] = (XVR_analog_reg_save[4] & ~(0xfffff)) | (wakeup_pin << 0) |(wakeup_pin << 5) | (wakeup_pin << 10) | (wakeup_pin << 15);
    }
}
...
//#endif
    ...
    os_printf("[System Env Feature:%08x]\r\n",env_h->env_cfg.feature.feature_flag);
...
int app_env_keypair_used_delete( btaddr_t *addr )
{
    ...
    for( i = 0; i < MAX_KEY_STORE; i++ )
    {
        ...
        && (btaddr_same( addr, &(env_h->env_data.key_pair[i].btaddr))) )
        {
            env_h->env_data.key_pair[i].used = 0x00;
            return 1;
        }
    }
    return 0;
...
{
    app_env_handle_t  env_h = app_env_get_handle();
    if((env_h->env_data.stereo_key.used != 0xFF)
        && (btaddr_same( addr, &(env_h->env_data.stereo_key.btaddr))) )
        {
            env_h->env_data.stereo_key.used = 0;
            return 1;
        }   
    return 0; 
}
...
void app_flash_read_env_data(void)
{
    ...
    for(i=0;i<8;i++) // 16
    {
        flash_read_data((uint8 *)&env_h->env_data, (uint32)FLASH_ENVDATA2_DEF_ADDR + i*ENV_BLOCK_LEN, sizeof(app_env_data_t));
...
void app_flash_write_env_data(void)
{
    ...
    uint8 buff[32] = {0};
    uint8 i = 0;
    os_printf("app_flash_write_env_data()\r\n");
    /* searching valid space for save this env data */
    for(i=0; i<8; i++) // 16
    {
        flash_read_data(buff, (uint32)FLASH_ENVDATA2_DEF_ADDR + i*ENV_BLOCK_LEN, 32);
        if(buff[0] == 0x5a)  // this block has been used 
        {
            buff[0] = 0;
            flash_write_data(buff,(uint32)FLASH_ENVDATA2_DEF_ADDR + i*ENV_BLOCK_LEN,1); // write 1st byte '0' to override this tag
        }
    ...
    if(i == 8) // 16 /* have not searched valid space,then erase this sector and write env data to 1st block; */
    {
        ...
        flash_erase_sector(FLASH_ENVDATA2_DEF_ADDR, FLASH_ERASE_4K);
        env_h->env_data.env_tag = 0x5a;
        flash_write_data( (uint8 *)&(env_h->env_data), FLASH_ENVDATA2_DEF_ADDR, sizeof(app_env_data_t));
    }
    else /* write env data to the i block */
    {
        env_h->env_data.env_tag = 0x5a;
        flash_write_data( (uint8 *)&(env_h->env_data), FLASH_ENVDATA2_DEF_ADDR + i*ENV_BLOCK_LEN, sizeof(app_env_data_t));
    }
}
...
int app_env_store_autoconn_info( btaddr_t * remote_addr,  uint8 link_key[16] )
{
    ...
    memcpy( (uint8 *)&key->linkkey, link_key, LINK_KEY_SIZE );
    memcpy( (uint8 *)&key->btaddr, (uint8 *)remote_addr, BYTE_BD_ADDR_SIZE );
...
{
    app_env_handle_t  env_h = app_env_get_handle();  
    return(&(env_h->env_cfg.bt_para.device_name[0]));      
...
//	app_env_dump();
		...
		&&app_env_check_feature_flag(APP_ENV_FEATURE_FLAG_AUX_MODE_PWR_DOWN)))
	 {
            ...
            //if(!app_env_check_feature_flag(APP_ENV_FEATURE_FLAG_PAIRING_AFTER_PWR_ON))
		{
            ...
            t0 = os_get_tick_counter();
				bt_flag2_operate(APP_FLAG2_5S_PRESS,1);
            while(1)
            {
            	...
            	{
							bt_flag2_operate(APP_FLAG2_5S_PRESS,0);
    ...
    {
        //gpio_config( 7, 5 ); // bk3265 ,GPIO3 input & pullup
        //gpio_config( DEFAULT_WAKEUP_PIN, 5 ); // bk3265 ,GPIO15.input & pullup
    #if (CONFIG_DEBUG_PCM_TO_UART == 1)
...
btaddr_t *app_env_get_stereo_btaddr(void)
{
    ...
    {
        return((btaddr_t *)(&key->btaddr));
    }
    return NULL;        
}
void app_env_set_stereo_info( btaddr_t *saddr, uint8 link_key[LINK_KEY_SIZE] )
{
    app_env_handle_t env_h = app_env_get_handle();
    app_env_key_pair_t *key = &env_h->env_data.stereo_key;
    os_printf("app_env_set_stereo_info:%d\r\n",get_tws_prim_sec());
    clear_tws_env_stereo_role();
    set_tws_env_stereo_role(get_tws_prim_sec());
    env_h->env_data.chg_Vctrl= get_tws_prim_sec();

    key->used = 1;
    memcpy( (uint8 *)&key->linkkey, link_key, LINK_KEY_SIZE );
    memcpy( (uint8 *)&key->btaddr, (uint8 *)saddr, BYTE_BD_ADDR_SIZE );
...
{
    app_env_handle_t env_h = app_env_get_handle();
    return (env_h->env_data.chg_Vctrl);
}

void app_env_update_stereo_role_info( uint8 role )
{
    set_tws_env_stereo_role(role);
}
#endif
...
#ifdef CONFIG_TWS
#if 0 //TWS_HFP_ENABLE
    if (APP_ENV_FEATURE_FLAG_AEC_ENABLE == flag
        && get_tws_prim_sec() == TWS_PRIM_SEC_SECOND)
    {
        return 0;
    }
#endif
#if CONFIG_CLEAR_MEMORY_EN
	if (APP_ENV_FEATURE_FLAG_PAIRING_AFTER_PWR_ON == flag)
		return 1;
	else
#endif
#endif
...
void app_env_pwrctrl_by_switch(void)
{
    if(SW_PWR_KEY_SWITCH==app_env_check_pwrCtrl_mode())
    {
        ...
        if((high_flag == gpio_input(pwrctrl_pin))
        #if ((CONFIG_CHARGE_EN == 1))
            || (bt_flag2_is_set(APP_FLAG2_CHARGE_POWERDOWN)&&get_Charge_state())
        #endif
        )
        {
            sw_io_cnt = 0;
...
void app_env_rf_pwr_set(uint8 type)
{
    ...
    else if(type == 1)
    {
        ...
        return;
    //os_printf("rf_pwr:0x%x,0x%x,0x%x\r\n",rf_pwr->big_adj,env_h->env_cfg.feature.a2dp_rf_pwr,env_h->env_cfg.feature.hfp_rf_pwr);
