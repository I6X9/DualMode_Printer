...
typedef enum
{
    W4_CHARGE_PREPARE_NONE = 0,
    W4_CHARGE_PREPARING    = 1,
    W4_CHARGE_PREPARED     = 2
}t_w4_battery_prepare;
typedef enum
{
    BATTERY_CHARGE_W4_SYS_BOOT = 0,
    BATTERY_CHARGE_PREPARE,
...
#define APP_FLAG_PROFILE_AUTO_CONN          0x00010000
#define APP_FLAG_FIRST_ENCRYP               0x00010000 // == APP_FLAG_PROFILE_AUTO_CONN
...
//#define APP_HID_MODE_FLAG			              0x08000000
...
typedef void (*bt_cb_function)(void) ;
...
typedef struct _system_s
{
    ...
    jtask_h  usb_host_enumerate_task;
    ...
    jtask_h 	app_tws_task;
    jtask_h 	tws_re_sync_task; /* only used for tws_re_sync procedure. */
    jtask_h 	tws_slave_check_task;
    jtask_h 	tws_linein_task;
    //jtask_h 	tws_del_pkt_check_task;
    jtask_h   a2dp_discover_task;
#endif
...
void app_bt_enable_qos(uint8 enable);
...
#define APP_ROLE_SWITCH_ENABLE          1
#define APP_ROLE_SWITCH_DISABLE         0
...
#ifdef CONFIG_TWS
#define APP_LAUNCH_PERIODIC_INQUIRY      0
#define APP_EXIT_PERIODIC_INQUIRY            1
...
int app_button_stereo_master_action(void);
int app_button_stereo_slave_action(void);
...
void app_set_crystal_calibration(uint8 val);
