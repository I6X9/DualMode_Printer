...
uint8 app_wave_file_play_start( int file_id )
{
...
#if TWS_HFP_ENABLE
	/* TWS_M and TWS_S don't play tip sound when they have sco voice. */
	if( bt_flag2_is_set( APP_FLAG2_STEREO_WORK_MODE ))
	{
		if ((get_tws_prim_sec() == TWS_PRIM_SEC_PRIMARY)&& bt_flag1_is_set(APP_FLAG_CALL_ESTABLISHED|APP_FLAG_HFP_CALLSETUP))
		{
		...
		{
			if (get_tws_hfp_flag(TWS_HFP_FLAG_SCO_CONN))
			{
				return WAV_NO_ERROR;
			}
		}
    ...
    if (bt_flag2_is_set(APP_FLAG2_STEREO_WORK_MODE))
    {
        if ((get_tws_prim_sec()==TWS_PRIM_SEC_SECOND)&& (file_id==APP_WAVE_FILE_ID_CONN)) 
            return WAV_POWER_OFF_WHILE_NOT_PWD;
...
// 同一时间来两提示音，若两个提示音不同类型，要重新初始化audio
...
#endif
    {
        ...
        {
            if ((file_conn_flag==0) && app_check_bt_mode(BT_MODE_1V1|BT_MODE_TWS))
                return WAV_NO_ERROR; 
            file_conn_flag = 0;			
        }
...
uint8 start_wave_and_action(uint32 file_id, wav_cb_func cb)
{
    ...
    {
        if(!bt_flag1_is_set(APP_FLAG_WAVE_PLAYING) && (cb))
			cb();		
		else    
...
void app_wave_file_play_clean(void)
{
    app_wave_handle_t wave_h = app_get_wave_handle();
    wave_h->flag_playing = 0;
    os_printf("app_wave_file_play_clean(%d)\n", wave_h->flag_playing);
    clear_wave_playing();
}
...
void app_wave_file_play_stop( void )
{
    ...
    os_printf("wav stop:%d,flag_sm:0x%x,0x%x\r\n",wave_h->flag_playing,app_h->flag_sm1,app_h->flag_sm2);
    if(wave_h->flag_playing == 0)
    {
        clear_wave_playing();
