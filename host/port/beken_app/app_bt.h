...
uint32 hci_get_acl_link_count(hci_unit_t *unit);
...
extern uint8_t tws_is_all_active(void);
#endif
...
#endif
#ifdef CONFIG_TWS
int hci_get_acl_link_check(hci_unit_t *unit,btaddr_t* raddr);
void bt_set_req_uclass(btclass_t uclass);
uint8 bt_get_req_uclass(void);
...
void app_write_supervision_timeout(btaddr_t *btaddr, uint16 to);
