...
typedef struct hci_link {
    ...
    uint8_t                     role;
    uint8_t                    support_role_switch;
#endif
#if (CONFIG_A2DP_PLAYING_SET_AG_FLOW == 1)
    uint8_t                    flow_ctrl;
#endif
#ifdef CONFIG_TWS
    uint8_t                     self_piconet_role;       /* 0: master; 1: slave; */
#endif
} hci_link_t;
...
/* hci_link.c */
void set_hci_acl_expiry(uint32 timeout_ms);
...
void hci_link_set_default_pkt_type(hci_write_default_pkt_type_cp *cp);
...
void unit_acl_disconnect_with_reason(hci_unit_t *unit, const btaddr_t *raddr, uint8 const reason);
#endif
